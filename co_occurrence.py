...
### SOLUTION BEGIN
    ...
    for document in corpus:
        set_of_distinct_words = set_of_distinct_words.union(document)
    corpus_words = sorted(list(set_of_distinct_words))
    num_corpus_words = len(set_of_distinct_words)
### SOLUTION END
...
### SOLUTION BEGIN
    ...
    for document in corpus:
        for center_idx in range(len(document)):
            start_win = (center_idx - window_size) if(center_idx - window_size > 0) else 0
            end_win   = (center_idx + window_size) if(center_idx + window_size < len(document)) else (len(document)-1)
            for context_idx in range(start_win, end_win + 1):
                if context_idx == center_idx:
                    continue
                M[word2Ind[document[center_idx]]][word2Ind[document[context_idx]]]+=1
### SOLUTION END
